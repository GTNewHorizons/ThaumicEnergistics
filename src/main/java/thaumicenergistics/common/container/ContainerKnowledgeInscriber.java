...
public class ContainerKnowledgeInscriber extends ContainerWithPlayerInventory {
    ...
    private static final int PLAYER_INV_POSITION_Y = 162,
            HOTBAR_INV_POSITION_Y = ContainerKnowledgeInscriber.PLAYER_INV_POSITION_Y + 58;
    ...
    public ContainerKnowledgeInscriber(final EntityPlayer player, final World world, final int x, final int y,
        ...
        this.bindPlayerInventory(
                player.inventory,
                ContainerKnowledgeInscriber.PLAYER_INV_POSITION_Y,
        ...
        this.kCoreSlot = new SlotRestrictive(
                this.inscriber,
                TileKnowledgeInscriber.KCORE_SLOT,
                ContainerKnowledgeInscriber.KCORE_SLOT_X,
    ...
    private CoreSaveState getSaveState() {
        ...
        // Is there a valid recipe?
            ...
            ItemStack recipeOutput = ArcaneRecipeHelper.INSTANCE.getRecipeOutput(
                    this.internalInventory,
                    ContainerKnowledgeInscriber.CRAFTING_MATRIX_SLOT,
                    ContainerKnowledgeInscriber.CRAFTING_GRID_SIZE,
                    this.activeRecipe);
    ...
    private void initCraftingSlots() {
        ...
        for (int row = 0; row < ContainerKnowledgeInscriber.CRAFTING_ROWS; row++) {
            for (int column = 0; column < ContainerKnowledgeInscriber.CRAFTING_COLS; column++) {
                ...
                int posX = ContainerKnowledgeInscriber.CRAFTING_SLOT_X
                        + (ContainerKnowledgeInscriber.CRAFTING_SLOT_SPACING * column);
                int posY = ContainerKnowledgeInscriber.CRAFTING_SLOT_Y
                        + (ContainerKnowledgeInscriber.CRAFTING_SLOT_SPACING * row);
    ...
    private void initPatternSlots() {
        ...
        for (int row = 0; row < ContainerKnowledgeInscriber.PATTERN_ROWS; row++) {
            for (int column = 0; column < ContainerKnowledgeInscriber.PATTERN_COLS; column++) {
                ...
                // Calculate the position
                int posX = ContainerKnowledgeInscriber.PATTERN_SLOT_X
                        + (ContainerKnowledgeInscriber.PATTERN_SLOT_SPACING * column);
                int posY = ContainerKnowledgeInscriber.PATTERN_SLOT_Y
                        + (ContainerKnowledgeInscriber.PATTERN_SLOT_SPACING * row);
    ...
    public void onClientRequestSaveOrDelete(final EntityPlayer player) {
        ...
        if (saveState == CoreSaveState.Enabled_Save) {
            ...
            // Get the aspect cost
            AspectList recipeAspects = ArcaneRecipeHelper.INSTANCE.getRecipeAspectCost(
                    this.internalInventory,
                    ContainerKnowledgeInscriber.CRAFTING_MATRIX_SLOT,
                    ContainerKnowledgeInscriber.CRAFTING_GRID_SIZE,
                    this.activeRecipe);
    ...
    public void onCraftMatrixChanged(final IInventory inv) {
        ...
        this.activeRecipe = ArcaneRecipeHelper.INSTANCE.findMatchingArcaneResult(
                inv,
                ContainerKnowledgeInscriber.CRAFTING_MATRIX_SLOT,
                ContainerKnowledgeInscriber.CRAFTING_GRID_SIZE,
                this.player);
        ...
        if (this.activeRecipe != null) {
            craftResult = ArcaneRecipeHelper.INSTANCE.getRecipeOutput(
                    inv,
                    ContainerKnowledgeInscriber.CRAFTING_MATRIX_SLOT,
                    ContainerKnowledgeInscriber.CRAFTING_GRID_SIZE,
                    this.activeRecipe);
