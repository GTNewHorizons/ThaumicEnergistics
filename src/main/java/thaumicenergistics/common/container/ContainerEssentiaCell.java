...
public class ContainerEssentiaCell extends ContainerEssentiaCellTerminalBase {
    ...
    /**
     * Compiler safe reference to the TileChest when using the ISaveProvider interface.
     */
    ...
    public ContainerEssentiaCell(final EntityPlayer player, final World world, final int x, final int y, final int z) {
        ...
        if (EffectiveSide.isServerSide()) {
            /*
             * Note: Casting the hostChest to an object is required to prevent the compiler from seeing the
             * soft-dependencies of AE2, such a buildcraft, which it attempts to resolve at compile time.
             */
    ...
    protected IMEEssentiaMonitor getNewMonitor() {
        try {
            ...
            if (handler != null) {
                ...
                return new EssentiaMonitor(
                        (IMEMonitor<IAEFluidStack>) handler,
                        this.hostChest.getProxy().getEnergy(),
                        this);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * Gets the current list from the AE monitor and sends it to the client.
     */
    @Override
    public void onClientRequestFullUpdate() {
        ...
        if (cellHandler != null) {
            ...
            Packet_C_EssentiaCellTerminal
                    .sendViewingModes(this.player, cellHandler.getSortingMode(), cellHandler.getViewMode());
        }
    ...
    public void onClientRequestViewModeChange(final EntityPlayer player, final boolean backwards) {
        ...
        if (cellHandler != null) {
            ...
            ViewItems viewMode = Platform
                    .rotateEnum(cellHandler.getViewMode(), backwards, Settings.VIEW_MODE.getPossibleValues());
