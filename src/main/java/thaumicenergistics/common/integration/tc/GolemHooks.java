...
public class GolemHooks {
    ...
    private static void logCaughtException(final String hook, final IGolemHookHandler handler, final Exception e) {
        if (handler != null) {
            if (e != null) {
                ThELog.warning(
                        "Caught Exception During call to '" + hook + "' for handler '%s':",
                        handler.getClass().getCanonicalName());
                ThELog.warning(e.toString());
            } else {
                ThELog.warning(
                        "Caught Unknown Exception During call to '" + hook + "' for handler '%s'",
                        handler.getClass().getCanonicalName());
            }
        } else if (e != null) {
            ...
            if (syncRegistry.hasChanged()) {
                if (GolemHooks.DATAWATCHER_ID > 0) {
                    try {
                        ...
                        try {
                            golem.getDataWatcher()
                                    .addObject(GolemHooks.DATAWATCHER_ID, syncRegistry.mappingsToString());
                        } catch (Exception e2) {
                            syncRegistry.markDirty();
                        }
                    }
