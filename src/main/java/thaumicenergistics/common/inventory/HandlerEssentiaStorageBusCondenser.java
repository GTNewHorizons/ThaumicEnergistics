...
public class HandlerEssentiaStorageBusCondenser extends HandlerEssentiaStorageBusBase {
    ...
    public IAEFluidStack injectItems(final IAEFluidStack input, final Actionable mode, final BaseActionSource source) {
        ...
        injectStack.setStackSize(
                500 * EssentiaConversionHelper.INSTANCE.convertFluidAmountToEssentiaAmount(input.getStackSize()));

        // Inject the fluid
        this.condenser.fill(
                this.partStorageBus.getSide().getOpposite(),
                injectStack.getFluidStack(),
