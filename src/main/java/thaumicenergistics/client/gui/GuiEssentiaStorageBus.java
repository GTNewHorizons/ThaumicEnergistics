...
public class GuiEssentiaStorageBus extends ThEBaseGui implements IAspectSlotGui {
    ...
    protected void onButtonClicked(final GuiButton button, final int mouseButton) {
        ...
        if (button.id == GuiEssentiaStorageBus.BUTTON_PRIORITY_ID) {
            ...
            Packet_S_ChangeGui.sendGuiChange(
                    ThEGuiHandler.generateSidedID(ThEGuiHandler.PRIORITY_ID, side),
                    this.player,
                    host.getWorldObj(),
                    host.xCoord,
                    host.yCoord,
            ...
            // Update the button
            ((GuiButtonAllowVoid) this.buttonList.get(GuiEssentiaStorageBus.BUTTON_ALLOW_VOID_ID))
                    .setIsVoidAllowed(this.isVoidAllowed);
